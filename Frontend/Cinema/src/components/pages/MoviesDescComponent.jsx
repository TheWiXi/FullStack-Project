...
export async function movieLoader ({params}) {
    ...
    const data = await fetch(`http://${import.meta.env.VITE_API}/cineapi/showPelicula?id=${params.id}`).then(res => res.json())
                ...
                {data && (
                    <section className={ `p-[10px] ${ cinema && 'mb-[100px]' }` }>
                        ...
                        <div className="flex max-w-full mt-[10px] pb-[20px] overflow-x-scroll">
                            {
                                data.cast.map(({_id, nombre, img, personaje}) => (
                                    <CastCards key={_id} img={img} name={nombre} rol={personaje} />
                                ))
                            }
                        </div>
