...
export const loadSeatReserved = async ({ params }) => {
    ...
    const { data } = await fetch(`http://${import.meta.env.VITE_API}/movie/${params.id}`, { cache: "force-cache" }).then(res => res.json())
...
export const SeatReservationScreen = () => {
    ...
    const onFinalizar = async () => {
        await fetch(`http://${import.meta.env.VITE_API}/ticket/${parametros.seatId}`, {method: 'DELETE'})
        Swal.fire({
            icon: "error",
            title: "Oops...",
    ...
    const comprar = async () => {
        await fetch(`http://${import.meta.env.VITE_API}/ticket/${parametros.seatId}`, {
            method: 'POST',
            headers: {
